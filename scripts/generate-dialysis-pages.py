...
def card(c, labels):
    ...
    return f'<article class="facility-card dialysis-card compact-card reveal" data-google-maps-url="{e(maps(c))}"{doctor_attribute}><h3>{e(c["name"])}</h3>{"".join(rows)}</article>'
...
def page(lang):
    ...
    home = {"fr": "", "ar": "ar.html", "en": "en.html"}[lang]
    ...
    return f'''<!DOCTYPE html><html lang="{lang}"{' dir="rtl"' if lang=='ar' else ''}><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>{e(title)} | Medomicile</title><meta name="description" content="{e(description)}"><meta property="og:url" content="{hrefs[lang]}"><link rel="canonical" href="{hrefs[lang]}">{''.join(f'<link rel="alternate" hreflang="{k}" href="{v}">' for k,v in hrefs.items())}<link rel="alternate" hreflang="x-default" href="{fr}"><link rel="stylesheet" href="style.css?v=20260908-01"><script type="application/ld+json">{{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{{"@type":"ListItem","position":1,"name":"{e(home)}","item":"{BASE + ("ar.html" if lang=="ar" else "en.html" if lang=="en" else "")}"}},{{"@type":"ListItem","position":2,"name":"{e(title)}","item":"{hrefs[lang]}"}}]}}</script></head><body>{header}<main id="main" class="page-main directory-main"><nav class="breadcrumb" aria-label="Breadcrumb"><a href="{BASE + ("ar.html" if lang=="ar" else "en.html" if lang=="en" else "")}">{e(home)}</a><span aria-hidden="true">›</span><span>{e(title)}</span></nav><section class="directory-hero page-hero section ambient-page-hero"><canvas class="ambient-canvas" data-ambient-canvas aria-hidden="true"></canvas><div class="directory-hero-copy reveal"><p class="eyebrow">{'دليل طبي محلي' if lang=='ar' else 'LOCAL MEDICAL DIRECTORY' if lang=='en' else 'NÉPHROLOGIE & HÉMODIALYSE'}</p><h1>{e(title)}</h1><p class="hero-subtitle">{e(description)}</p><p class="directory-note">{'المعلومات المقدمة إرشادية وقد تتغير. يرجى الاتصال بالمركز قبل التنقل.' if lang=='ar' else 'The information presented is indicative and may change. Contact the center before going.' if lang=='en' else 'Les informations présentées sont données à titre indicatif et peuvent évoluer. Contactez directement le centre avant de vous déplacer.'}</p></div></section>{featured_html}<section class="directory section"><div class="pharmacy-panel reveal"><div class="pharmacy-copy"><h2>{'Autres centres de dialyse à Kénitra' if featured else e(title)}</h2></div><div class="facility-grid dialysis-grid">{cards}</div></div></section></main>{footer}<a dir="ltr" class="floating-call" href="tel:+212663058222">{e(call)}</a><script defer src="script.js?v=20260908-01"></script></body></html>'''
